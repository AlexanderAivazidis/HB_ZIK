...
---
  ### Finding gene modules that make cells susceptibel to Zika infection
  
  ---
  title: "Finding gene modules that make cells susceptibel to Zika infection"
output:
  html_document: default
...
data_subset = as.matrix(readMM(paste('../data/ZikaGlioblastomas/cellranger302_count_32771_5953STDY855119', as.character(i), '_GRCh38-3_0_0_premrna/filtered_feature_bc_matrix/filtered_feature_bc_matrix/matrix.mtx', sep = '')))
geneNames = read.table(paste('../data/ZikaGlioblastomas/cellranger302_count_32771_5953STDY855119', as.character(i), '_GRCh38-3_0_0_premrna/filtered_feature_bc_matrix/filtered_feature_bc_matrix/features.tsv', sep = ''))
sampleNames = read.table(paste('../data/ZikaGlioblastomas/cellranger302_count_32771_5953STDY855119', as.character(i), '_GRCh38-3_0_0_premrna/filtered_feature_bc_matrix/filtered_feature_bc_matrix/barcodes.tsv', sep = ''))
rownames(data_subset) = geneNames[,1]
colnames(data_subset) = sampleNames[,1]
data_subset = data_subset[rownames(glioblastoma_counts),]
...
newNames = geneNames[match(rownames(glioblastoma_counts), geneNames[,1]),2]
replace = !is.na(newNames)
rownames(glioblastoma_counts)[replace] = as.character(newNames[replace])
...
Based on the JackStraw procedure I select 10 PCs for further analysis:
